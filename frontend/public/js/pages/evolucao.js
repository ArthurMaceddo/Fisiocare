...
Pages.evolucao = async function(container) {
  container.innerHTML = `
    ...
    <div class="page-body" style="padding:0;height:calc(100vh - 66px);overflow:hidden">
      <div class="evolucao-grid">
        ...
        <div class="card paciente-lista" style="border-radius:0;box-shadow:none;border-right:1px solid var(--border)">
          <div class="card-header" style="position:sticky;top:0;z-index:1;background:#fff">
            <h3>Pacientes</h3>
          </div>
    ...
    <!-- Modal Nova Sessão -->
    <div id="modal-sessao" class="modal-overlay" style="display:flex">
      <div class="modal">
        ...
        <div class="form-grid">
          ...
          <div class="form-group full">
            <label>Descrição do Tratamento Realizado *</label>
            <textarea id="sess-desc" placeholder="Descreva os procedimentos realizados..."></textarea>
          </div>
          <div class="form-group full">
            <label>Exercícios Realizados</label>
            <textarea id="sess-exerc" placeholder="Liste os exercícios realizados na sessão..."></textarea>
          </div>
          <div class="form-group full">
            <label>Avaliação do Fisioterapeuta</label>
            <textarea id="sess-aval" placeholder="Avaliação clínica do profissional..."></textarea>
          </div>
          <div class="form-group">
          ...
          <div class="form-group full">
            ...
            <textarea id="sess-obs" placeholder="Outras observações relevantes..."></textarea>
          </div>
        </div>
...
const Evolucao = {
  ...
  async selecionarPaciente(id) {
    ...
    detalhe.innerHTML = '<div class="loader"><div class="spinner"></div> Carregando evolução...</div>';

    try {
      const sessoes = await Api.sessoes.porPaciente(id);
      this.renderDetalhe(this.pacienteSelecionado, sessoes);
    } catch(e) {
      detalhe.innerHTML = `<div class="empty-state"><div class="icon">⚠️</div><p>${e.message}</p></div>`;
    }
  },

  renderDetalhe(p, sessoes) {
    const realizadas = sessoes.length;
    const agend = null; // futuro: buscar agendamento ativo para calcular faltantes
    ...
    detalhe.innerHTML = `
      ...
      <div class="card" style="padding:20px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          ...
          <button class="btn btn-primary btn-sm" onclick="Evolucao.abrirModalSessao(${p.id})">
            + Nova Sessão
          </button>
        </div>
        ...
        <div style="display:flex;gap:16px;margin-top:16px">
          <div style="background:var(--accent);border-radius:8px;padding:10px 16px;text-align:center">
            <strong style="font-size:1.4rem;color:var(--primary)">${realizadas}</strong>
  ...
  async abrirModalSessao(pacienteId) {
    ...
    } catch(e) { /* ignora, agendamento opcional */ }
    ...
    ['sess-mob-antes','sess-mob-depois','sess-evolucao'].forEach(id => document.getElementById(id).value = '');
    ['sess-desc','sess-exerc','sess-aval','sess-obs'].forEach(id => document.getElementById(id).value = '');
  ...
  async salvarSessao() {
    ...
    if (!data || !desc) {
      Toast.show('Preencha data e descrição do tratamento.', 'error'); return;
    }
    if (!agendId) {
      Toast.show('Selecione o agendamento.', 'error'); return;
    }
